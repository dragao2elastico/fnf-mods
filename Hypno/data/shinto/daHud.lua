--[[
	...
	hii raltyro this is grape byo!1!1... :3
	waldi i will destropy yuor super gaming house if
	you ask me for codign help agian.... :33
...
--]] local settings = {
    ...
    smoothCamera = true,
    smoothZoom = true,
    smoothIntensity = .7, -- too much smooth or below 0 = camera wont move lmao
...
function onCreatePost()
    ...
    if (settings.hideTimeBar) == false then

        -- CUSTOM TIME BAR BG --
        makeLuaSprite("timeBG")
        makeGraphic("timeBG", screenWidth, 10, "000000")

        setObjectCamera("timeBG", "other")
        setScrollFactor("timeBG", 0, 0)

        -- CUSTOM TIME BAR PROGRESS --
        makeLuaSprite("timeWhite")
        makeGraphic("timeWhite", screenWidth, 10, "FFFFFF")

        setObjectCamera("timeWhite", "other")
        setScrollFactor("timeWhite", 0, 0)

        setProperty("timeWhite.x", -screenWidth)

        -- adding the custom time bar
        addLuaSprite("timeBG")
        addLuaSprite("timeWhite")

        local songSeconds = getSongLength()

        -- CURRENT SONG TIME TEXT --

        makeLuaText("currentTime", "0:00", 1, 5, 50)
        setTextAlignment("currentTime", "left")
        setTextFont("currentTime", dFont)
        setTextSize("currentTime", 22)

        setProperty("currentTime.wordWrap", false)
        setProperty("currentTime.autoSize", true)

        setProperty("currentTime.antialiasing", gAA)

        setScrollFactor("currentTime", 0, 0)
        setObjectCamera("currentTime", "other")

        -- SONG LENGTH TEXT --
        local minutesLeft = tostring(math.floor(songSeconds / 60))
        local secondsLeft = tostring(songSeconds - minutesLeft * 60)

        makeLuaText("totalTime", "", 1, screenWidth - 60, 20)
        setTextAlignment("totalTime", "right")
        setTextFont("totalTime", dFont)
        setTextSize("totalTime", 22)

        setProperty("totalTime.wordWrap", false)
        setProperty("totalTime.autoSize", true)

        setProperty("totalTime.antialiasing", gAA)

        setScrollFactor("totalTime", 0, 0)
        setObjectCamera("totalTime", "other")

        -- adding the text thingies
        addLuaText("currentTime")
        addLuaText("totalTime")

        if not downscroll then -- if upscroll

            setProperty("timeBG.y", screenHeight - 10)
            setProperty("timeWhite.y", screenHeight - 10)

            setProperty("currentTime.y", screenHeight - 38)
            setProperty("totalTime.y", getProperty('currentTime.y'))

            setProperty("songName.y", screenHeight / 2 + 320)
    ...
    if (settings.showHealthPercent) then
        ...
        setObjectCamera("healthper", "other")
...
function onSongStart()
    doTweenX("timeBarTween", "timeWhite", 0, getSongLength(), "linear")

    runTimer("timeSecondLoop", 1, getSongLength())
end
        ...
        if (wowCombo >= 5 and not dontPlayCombo) then
            ...
            for i = 0, getProperty("notes.length") - 1 do
                ...
                if (getPropertyFromGroup("notes", i, "mustPress") and
                    not getPropertyFromGroup("notes", i, "isSustainNote") and pos >= songPos and pos < et) then
                    n = n + 1
                end
            end
        end
...
function onUpdatePost(dt)
    ...
    local function getHitsOfRating(x)
        return accuracyHitPercentage[x] * getProperty(x)
    end

    local accuracyHitBased = ((getHitsOfRating('sicks') + getHitsOfRating('goods') + getHitsOfRating('bads') +
                                 getHitsOfRating('shits')) / (getProperty('songHits') + getProperty('songMisses'))) /
                                 100

    local scoreTxt = "Score: " .. tostring(getProperty("songScore"))
    local missesTxt = "Misses: " .. tostring(getProperty("songMisses"))
    local ratingTxt = "Rating: " .. (ratingName == "?" and "?" or
                          (ratingName .. " (" .. tostring(math.truncate(accuracyHitBased * 100, 2)) .. "%)" .. " - " ..
    ...
    if (settings.coolBar) then
        ...
        setObjectClip("healthBarP2", vvv, 0,
            math.clamp(math.lerp(0, getProperty("healthBarP2.frame.frame.width"), health / 1), 1, 924), -1)
